...
layout(set = 0, binding = 0, std430) buffer ReadBuffer {
	uint data[];  // Each uint contains 4 bytes (RGBA packed)
} read_buffer;

layout(set = 0, binding = 1, std430) buffer WriteBuffer {
	uint data[];  // Each uint contains 4 bytes (RGBA packed)
} write_buffer;
...
void main() {
	...
	if (pixel_idx >= read_buffer.data.length()) return;

	uint pixel = read_buffer.data[pixel_idx];
	uint r = (pixel >> 0)  & 0xFF;
	uint g = (pixel >> 8)  & 0xFF;
	...
	for (int dx = -1; dx <= 1; dx++) {
		for (int dy = -1; dy <= 1; dy++) {
			...
			if (nx >= 0 && nx < int(push_constants.width) && ny >= 0 && ny < int(push_constants.height)) {
				...
				uint neighbor_pixel = read_buffer.data[neighbor_idx];
